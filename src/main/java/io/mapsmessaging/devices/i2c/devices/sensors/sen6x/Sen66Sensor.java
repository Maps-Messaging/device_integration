...
import io.mapsmessaging.devices.i2c.devices.sensors.sen6x.commands.Sen66MeasurementManager;
...
public class Sen66Sensor extends Sen6xSensor {
  public Sen66Sensor(AddressableDevice device) {
    super(device);
  }
  ...
  protected Sen6xMeasurementManager contructMeasurementManager(Sen6xCommandHelper helper) {
    return new Sen66MeasurementManager(helper);
  }
  ...
  protected List<SensorReading<?>> buildMeasurementReadingds(Sen6xMeasurementManager manager) {
    ...
    list.add(new AirQualityLevelCommand(manager));
